...
from MuXTalk_functions_Streamlit_Docker_v2 import *
...
if (custom_GRN_edges is None) & (custom_PPI_edges is None) & (custom_GRN_parquet is None):
	...
	if MuXTalk_method == 'MuXTalk_between':
		...
		st.session_state['detected_ranked_pathway_pairs_df'] = pd.read_parquet(args.proj_path + '%s_%s_between_detected_discovery.parquet' % (input_GRN, input_PPI))
	...
	st.error('If entering a custom GRN, please ensure that all files are provided for the custom GRN.')
	st.stop()
...
with tips_expander:
	...
	- To "cool down" the networks, i.e. keep the nodes from moving, you can decrease gravitational constant \
