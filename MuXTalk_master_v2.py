...
def process_GRN_cisbp_data(proj_path, input_filenames_dict, input_refseq,  output_GRN):
...
def randomize_GRN(A_GRN_sparr, proj_path, input_GRN, input_PPI, N_runs=500, N_swap=10, return_output=False):
    
    if not os.path.exists(proj_path + '%s_%s_A_GRN_sparr_rand_dict_%sruns.pickle' % (input_GRN, input_PPI, N_runs)):
        ...
        with open(proj_path + '%s_%s_A_GRN_sparr_rand_dict_%sruns.pickle' % (input_GRN, input_PPI, N_runs), 'wb') as fp:
        ...
        with open(proj_path + '%s_%s_A_GRN_sparr_rand_dict_%sruns.pickle' % (input_GRN, input_PPI, N_runs), 'rb') as fp:
            A_GRN_sparr_rand_dict = pickle.load(fp)        

        if return_output==True:        
            return A_GRN_sparr_rand_dict

def randomize_KEGG_e(A_KEGG_e_sparr_dict, KEGG_interaction_types_dict, proj_path, input_PPI, N_runs=500, N_swap=10, return_output=False):

    if not os.path.exists(proj_path + '%s_A_KEGG_e_sparr_rand_dict_%sruns.pickle' % (input_PPI, N_runs)):    
        ...
        with open(proj_path + '%s_A_KEGG_e_sparr_rand_dict_%sruns.pickle' % (input_PPI, N_runs), 'wb') as fp:
        ...
        with open(proj_path + '%s_A_KEGG_e_sparr_rand_dict_%sruns.pickle' % (input_PPI, N_runs), 'rb') as fp:
            A_KEGG_e_sparr_rand_dict = pickle.load(fp)        
        
        if return_output==True:
            return A_KEGG_e_sparr_rand_dict        

def randomize_KEGGPPI(KEGG_PPI_allnodes_entrez, KEGG_interaction_types_dict, A_KEGG_e_sparr_rand_dict_dir, proj_path, input_PPI, return_output=False):
    
    if not os.path.exists(proj_path + '%s_A_KEGGPPI_sparr_rand_dict_%sruns.pickle' % (input_PPI, A_KEGG_e_sparr_rand_dict_dir.split('runs')[0].split('_')[-1])):  
        ...
        with open(proj_path + '%s_A_KEGGPPI_sparr_rand_dict_%sruns.pickle' % (input_PPI, len(A_KEGG_e_sparr_rand_dict)), 'wb') as fp:
        ...
        with open(proj_path + '%s_A_KEGGPPI_sparr_rand_dict_%sruns.pickle' % (input_PPI, A_KEGG_e_sparr_rand_dict_dir.split('runs')[0].split('_')[-1]), 'rb') as fp:
...
def count_motifs(A_mux, all_motif_types_list):
    ...
    if '00' in motif_counts_hashed.keys():
    	motif_counts_hashed['00'] = int(motif_counts_hashed['00'] / 2)    
    if '20' in motif_counts_hashed.keys():
        motif_counts_hashed['20'] = int(motif_counts_hashed['20'] / 2)
    if '90' in motif_counts_hashed.keys():
        motif_counts_hashed['90'] = int(motif_counts_hashed['90'] / 2)    
...
def between_paths_multilink_counts(common_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, 
    ...
    if not os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts.pickle' % (input_GRN, input_PPI)):        
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts.pickle' % (input_GRN, input_PPI), 'wb') as fp:
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts.pickle' % (input_GRN, input_PPI), 'rb') as fp:
...
def between_paths_multilink_counts_rand(A_GRN_sparr_rand_dict, common_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, 
                                        all_motif_types,A_KEGGPPI_sparr_rand_dict, all_motif_types_list, KEGG_interaction_types_dict, 
                                        A_KEGG_e_sparr_rand_dict, proj_path, input_GRN, input_PPI, rand_start = 0, N_rand = 100, N_swap = 10):

    if not os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, N_rand)):     
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, N_rand), 'wb') as fp:
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, N_rand), 'rb') as fp:
...
def between_paths_multilink_zscores_pvals(common_crosstalk_paths, between_paths_multilink_counts_rand_df_dict, 
                                          between_paths_multilink_counts_df_dict, all_motif_types, proj_path, input_GRN, input_PPI, N_rand=100):

    if not (os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI)) &
            os.path.exists(proj_path + '%s_%s_between_paths_rand_zscores.pickle' % (input_GRN, input_PPI))):       
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI), 'wb') as fp:
            pickle.dump(between_paths_multilink_counts_stats_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
        with open(proj_path + '%s_%s_between_paths_rand_zscores.pickle' % (input_GRN, input_PPI), 'wb') as fp:
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI), 'rb') as fp:
            between_paths_multilink_counts_stats_df_dict = pickle.load(fp)
        with open(proj_path + '%s_%s_between_paths_rand_zscores.pickle' % (input_GRN, input_PPI), 'rb') as fp:
...
def get_shortest_paths(A_KEGGPPI_sparr, common_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, proj_path, input_PPI,
                       sp_threshold=None):

    if not (os.path.exists(proj_path + '%s_sp%s_shortest_path_edges_dict.pickle' % (input_PPI, sp_threshold)) &
            os.path.exists(proj_path + '%s_sp%s_shortest_path_intermediaries_dict.pickle' % (input_PPI, sp_threshold))):     
        ...
        with open(proj_path + '%s_sp%s_shortest_path_edges_dict.pickle' % (input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_path_edges_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
        with open(proj_path + '%s_sp%s_shortest_path_intermediaries_dict.pickle' % (input_PPI, sp_threshold), 'wb') as fp:
        ...
        with open(proj_path +'%s_sp%s_shortest_path_edges_dict.pickle' % (input_PPI, sp_threshold), 'rb') as fp:
            shortest_path_edges_dict = pickle.load(fp)
        with open(proj_path + '%s_sp%s_shortest_path_intermediaries_dict.pickle' % (input_PPI, sp_threshold), 'rb') as fp:
...
def shortest_paths_multilink_counts(common_crosstalk_paths, shortest_path_edges_dict, sp_threshold,
    ...
    if not os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts.pickle' % (input_GRN, input_PPI, sp_threshold)):  
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts.pickle' % (input_GRN, input_PPI, sp_threshold), 'wb') as fp:
        ...
        print('Reading multilink counts of shortest path edges with threshold (%s) between all pairs of KEGG signaling pathways...' 
              % str(sp_threshold))
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts.pickle' % (input_GRN, input_PPI, sp_threshold), 'rb') as fp:
...
def shortest_paths_multilink_counts_rand(A_GRN_sparr_rand_dict, common_crosstalk_paths, shortest_path_edges_dict, sp_threshold,
    ...
    if not os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand)):     
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand), 'wb') as fp:
        ...
        print('Reading multilink counts of shortest path edges with threshold (%s) between all pairs of KEGG signaling pathways for randomized multilayer network ensebles...' 
              % str(sp_threshold))        
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand), 'rb') as fp:
...
def shortest_paths_multilink_zscores_pvals(common_crosstalk_paths, shortest_paths_multilink_counts_rand_df_dict, sp_threshold,
                                          shortest_paths_multilink_counts_df_dict, all_motif_types, proj_path, input_GRN, input_PPI, N_rand=100):

    if not (os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI, sp_threshold)) &
            os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores.pickle' % (input_GRN, input_PPI, sp_threshold))):         
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_paths_multilink_counts_stats_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
        with open(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores.pickle' % (input_GRN, input_PPI, sp_threshold), 'wb') as fp:
        ...
        print('Reading multilink statistics for shortest path edges with threshold (%s) between all pairs of KEGG signaling pathways...' 
              % str(sp_threshold))
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats.pickle' % (input_GRN, input_PPI, sp_threshold), 'rb') as fp:
            shortest_paths_multilink_counts_stats_df_dict =  pickle.load(fp)  
        with open(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores.pickle' % (input_GRN, input_PPI, sp_threshold), 'rb') as fp:
...
def between_paths_multilink_counts_discovery(discovery_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, 
    ...
    if not os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts_discovery.pickle' % (input_GRN, input_PPI)):    
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_discovery.pickle' % (input_GRN, input_PPI), 'wb') as fp:
            pickle.dump(between_paths_multilink_counts_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
            
    else:
        
        print('Reading multilink counts of direct edges between all pairs of KEGG signaling pathways...')  
        with open(proj_path + '%s_%s_between_paths_multilink_counts_discovery.pickle' % (input_GRN, input_PPI), 'rb') as fp:
            between_paths_multilink_counts_df_dict = pickle.load(fp)        
...
def between_paths_multilink_counts_rand_discovery(A_GRN_sparr_rand_dict, discovery_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, 
                                        all_motif_types,A_KEGGPPI_sparr_rand_dict, all_motif_types_list, KEGG_interaction_types_dict, 
                                        A_KEGG_e_sparr_rand_dict, proj_path, input_GRN, input_PPI, N_rand = 100, N_swap = 10):

    if not os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, N_rand)):     
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, N_rand), 'wb') as fp:
            pickle.dump(between_paths_multilink_counts_rand_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
            
    else:
        
        print('Reading multilink counts of direct edges between all pairs of KEGG signaling pathways for randomized multilayer network ensebles...')  
        with open(proj_path + '%s_%s_between_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, N_rand), 'rb') as fp:
            between_paths_multilink_counts_rand_df_dict = pickle.load(fp)         
...
def between_paths_multilink_zscores_pvals_discovery(discovery_crosstalk_paths, between_paths_multilink_counts_rand_df_dict, 
    ...
    if not (os.path.exists(proj_path + '%s_%s_between_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI)) &
            os.path.exists(proj_path + '%s_%s_between_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI))):       
        ...
        with open(proj_path + '%s_%s_between_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI), 'wb') as fp:
            pickle.dump(between_paths_multilink_counts_stats_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
        with open(proj_path + '%s_%s_between_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI), 'wb') as fp:
            pickle.dump(between_paths_rand_zscores_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
            
    else:
        
        print('Reading multilink statistics for direct edges between all pairs of KEGG signaling pathways...')
        with open(proj_path + '%s_%s_between_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI), 'rb') as fp:
            between_paths_multilink_counts_stats_df_dict = pickle.load(fp)  
        with open(proj_path + '%s_%s_between_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI), 'rb') as fp:
            between_paths_rand_zscores_df_dict = pickle.load(fp)          

    return (between_paths_multilink_counts_stats_df_dict, between_paths_rand_zscores_df_dict)

def get_shortest_paths_discovery(A_KEGGPPI_sparr, discovery_crosstalk_paths, KEGG_PPI_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, 
								proj_path, input_PPI, sp_threshold=None):

    if not (os.path.exists(proj_path + '%s_sp%s_shortest_path_edges_dict_discovery.pickle' % (input_PPI, sp_threshold)) &
            os.path.exists(proj_path + '%s_sp%s_shortest_path_intermediaries_dict_discovery.pickle' % (input_PPI, sp_threshold))):     
        ...
        with open(proj_path + '%s_sp%s_shortest_path_edges_dict_discovery.pickle' % (input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_path_edges_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
        with open(proj_path + '%s_sp%s_shortest_path_intermediaries_dict_discovery.pickle' % (input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_path_intermediaries_dict, fp, protocol=pickle.HIGHEST_PROTOCOL) 
        ...
        with open(proj_path + '%s_sp%s_shortest_path_edges_dict_discovery.pickle' % (input_PPI, sp_threshold), 'rb') as fp:
            shortest_path_edges_dict = pickle.load(fp)
        with open(proj_path + '%s_sp%s_shortest_path_intermediaries_dict_discovery.pickle' % (input_PPI, sp_threshold), 'rb') as fp:
            shortest_path_intermediaries_dict = pickle.load(fp) 
...
def shortest_paths_multilink_counts_rand_discovery(A_GRN_sparr_rand_dict, discovery_crosstalk_paths, shortest_path_edges_dict, sp_threshold,
    ...
    if not os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand)):     
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand), 'wb') as fp:
            pickle.dump(shortest_paths_multilink_counts_rand_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)
        ...
        print('Reading multilink counts of shortest path edges with threshold (%s) between all pairs of KEGG signaling pathways for randomized multilayer network ensebles...' 
              % str(sp_threshold))
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_rand_%srands_discovery.pickle' % (input_GRN, input_PPI, sp_threshold, N_rand), 'rb') as fp:
            shortest_paths_multilink_counts_rand_df_dict = pickle.load(fp)        
...
def shortest_paths_multilink_zscores_pvals_discovery(discovery_crosstalk_paths, shortest_paths_multilink_counts_rand_df_dict, sp_threshold,
    ...
    if not (os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI, sp_threshold)) &
            os.path.exists(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI, sp_threshold))):         
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_paths_multilink_counts_stats_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
        with open(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI, sp_threshold), 'wb') as fp:
            pickle.dump(shortest_paths_rand_zscores_df_dict, fp, protocol=pickle.HIGHEST_PROTOCOL)  
        ...
        with open(proj_path + '%s_%s_sp%s_shortest_paths_multilink_counts_stats_discovery.pickle' % (input_GRN, input_PPI, sp_threshold), 'rb') as fp:
            shortest_paths_multilink_counts_stats_df_dict = pickle.load(fp)  
        with open(proj_path + '%s_%s_sp%s_shortest_paths_rand_zscores_discovery.pickle' % (input_GRN, input_PPI, sp_threshold), 'rb') as fp:
            shortest_paths_rand_zscores_df_dict = pickle.load(fp)          
...
def node_overlap_sig(KEGG_allnodes_entrez_df, KEGG_path_nodes_entrez_dict, KEGG_PPI_all_edges_entrez, KEGG_all_paths, common_crosstalk_paths, proj_path):
...
def auroc_auprc(ranked_pathway_pairs, ranked_pathway_pairs_df, len_sig, common_crosstalk_paths, XTalk_DB_positives_common_str, nonsig_Nrand = 1000):
    ...
    return roc_pr_stats, AUROC, AUPRC
...
def get_ranked_pathway_pairs_benchmark(XTalk_DB_positives_common_str, common_crosstalk_paths, proj_path, method='MuXTalk_shortest',
                             multilink_params={'GRN': 'HumanGRN10e6', 'PPI': 'PPI_Cheng_NatComms2019', 'sp_threshold': 1}, 
                             XTalk_params={'K': 1, 'Nrand': 100}):
    
    if method == 'MuXTalk_between':
        
        with open(proj_path + '%s_%s_between_paths_multilink_counts_stats.pickle' % (multilink_params['GRN'], multilink_params['PPI']), 'rb') as fp:
            between_paths_multilink_counts_stats_df_dict = pickle.load(fp)
        ...
        with open(proj_path + 
                  '%s_%s_sp%s_shortest_paths_multilink_counts_stats.pickle' % (multilink_params['GRN'], multilink_params['PPI'], 
                  																multilink_params['sp_threshold']), 'rb') as fp:
            shortest_paths_multilink_counts_stats_df_dict = pickle.load(fp)
...
def get_ranked_pathway_pairs_discovery(XTalk_DB_positives_common_str, proj_path, method='MuXTalk_shortest', 
                                       multilink_params={'GRN': 'HumanGRN10e6', 'PPI': 'PPI_Cheng_NatComms2019', 'sp_threshold': 1}, parquet=False):
    
    if method == 'MuXTalk_between':
        
        if not os.path.exists(proj_path + '%s_%s_between_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'])):
        
            with open(proj_path + '%s_%s_between_paths_multilink_counts_stats_discovery.pickle' % (multilink_params['GRN'], multilink_params['PPI']), 'rb') as fp:
                between_paths_multilink_counts_stats_df_dict = pickle.load(fp)
            (between_multilink_pos, between_multilink_pos_all) = multilink_positives(between_paths_multilink_counts_stats_df_dict)
            ...
            if parquet==True:
                ranked_pathway_pairs_df.to_parquet(proj_path + '%s_%s_between_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI']))
                detected_ranked_pathway_pairs_df.to_parquet(proj_path + '%s_%s_between_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI']))
            else:
                ranked_pathway_pairs_df.to_csv(proj_path + '%s_%s_between_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI']))
                detected_ranked_pathway_pairs_df.to_csv(proj_path + '%s_%s_between_detected_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI']))                
        
        else:
            
            if parquet==True:
                ranked_pathway_pairs_df = pd.read_parquet(proj_path + '%s_%s_between_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI']))
                detected_ranked_pathway_pairs_df = pd.read_parquet(proj_path + '%s_%s_between_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI']))
            else:
                ranked_pathway_pairs_df = pd.read_csv(proj_path + '%s_%s_between_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI']), index_col=0)
                detected_ranked_pathway_pairs_df = pd.read_csv(proj_path + '%s_%s_between_detected_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI']), index_col=0)  
        ...
        if not os.path.exists(proj_path + '%s_%s_sp%s_shortest_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold'])):
            
            with open(proj_path + 
                      '%s_%s_sp%s_shortest_paths_multilink_counts_stats_discovery.pickle' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']), 'rb') as fp:
                shortest_paths_multilink_counts_stats_df_dict = pickle.load(fp)
            (shortest_multilink_pos, shortest_multilink_pos_all) = multilink_positives(shortest_paths_multilink_counts_stats_df_dict)
            ...
            if parquet==True:
                ranked_pathway_pairs_df.to_parquet(proj_path + '%s_%s_sp%s_shortest_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))
                detected_ranked_pathway_pairs_df.to_parquet(proj_path + '%s_%s_sp%s_shortest_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))
            else:
                ranked_pathway_pairs_df.to_csv(proj_path + '%s_%s_sp%s_shortest_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))
                detected_ranked_pathway_pairs_df.to_csv(proj_path + '%s_%s_sp%s_shortest_detected_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))            
            
        else:
            
            if parquet==True:
                ranked_pathway_pairs_df = pd.read_parquet(proj_path + '%s_%s_sp%s_shortest_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))
                detected_ranked_pathway_pairs_df = pd.read_parquet(proj_path + '%s_%s_sp%s_shortest_detected_discovery.parquet' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']))
            else:
                ranked_pathway_pairs_df = pd.read_csv(proj_path + '%s_%s_sp%s_shortest_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']), index_col=0)
                detected_ranked_pathway_pairs_df = pd.read_csv(proj_path + '%s_%s_sp%s_shortest_detected_discovery.csv' % (multilink_params['GRN'], multilink_params['PPI'], multilink_params['sp_threshold']), index_col=0)             
...
def run_MuXTalk(proj_path, input_filenames_dict, input_GRN, input_PPI, MuXTalk_method='MuXTalk_shortest', N_runs=500, N_swap=10, sp_threshold=1, 
    ...
    (KEGG_PPI_allnodes_entrez, GRN_KEGG_PPI_edges, PPI_all_edges_entrez, KEGG_PPI_all_edges_entrez, KEGG_PPI_allnodes_entrez_df, 
     KEGG_interaction_types_dict, KEGG_all_edges_entrez, KEGG_all_paths, KEGG_path_nodes_entrez_dict, 
     all_motif_types, all_motif_types_list) = process_data(proj_path, input_PPI, input_PPI_ID_format, input_GRN, input_GRN_ID_format, input_filenames_dict)
    ...
    A_KEGG_e_sparr_rand_dict = randomize_KEGG_e(A_KEGG_e_sparr_dict, KEGG_interaction_types_dict, proj_path, N_runs=N_runs, 
...
def get_ranked_pathway_pairs_LOLO(XTalk_DB_positives_common_str, common_crosstalk_paths, KEGG_interaction_types_dict, proj_path, method='MuXTalk_shortest',
                             multilink_params={'GRN': 'HumanGRN10e6', 'PPI': 'PPI_Cheng_NatComms2019', 'sp_threshold': 1}, 
                             XTalk_params={'K': 1, 'Nrand': 100}):

    ranked_pathway_pairs_df_dict = {}
    ranked_pathway_pairs_dict = {}
    detected_ranked_pathway_pairs_df_dict = {}
    detected_ranked_pathway_pairs_dict = {}
    len_sig_dict = {}
    ...
    if method == 'MuXTalk_between':
        ...
        num_sig_multilinks = pd.DataFrame(index=['%s-%s' % (i, j) for i, j in all_perms], columns=['Pathway A', 'Pathway B', '# sig', 
                                                                                                   ...
                                                                                                   'z-scores', 'best z-score', 'p-values',
                                                                                                  'best p-value', 'MuXTalk_score', 'in gold'])
        for excluded_layer in tqdm(KEGG_interaction_types_dict.keys(), position=0, leave=True):

            for (p1, p2) in all_perms: 
                temp_df = between_paths_multilink_counts_stats_df_dict[(p1, p2)].loc[set(between_multilink_pos.keys()) - 
                                                                                      {'%s1' % KEGG_interaction_types_dict[excluded_layer], 
                                                                                       '%s-1' % KEGG_interaction_types_dict[excluded_layer]}]
                num_sig_multilinks.at['%s-%s' % (p1, p2), 'Pathway A'] = p1
                num_sig_multilinks.at['%s-%s' % (p1, p2), 'Pathway B'] = p2
                num_sig_multilinks.at['%s-%s' % (p1, p2), '# sig'] = len(temp_df[(temp_df['actual_motif_counts'] - temp_df['rand_mean'] > 0) & 
                                                                                 (temp_df['p-value']<=0.05)])
            ...
            ranked_pathway_pairs_df_dict[excluded_layer] = num_sig_multilinks.sort_values(['MuXTalk_score'], ascending=False)
            ranked_pathway_pairs_dict[excluded_layer] = ranked_pathway_pairs_df_dict[excluded_layer].index
            detected_ranked_pathway_pairs_df_dict[excluded_layer] = ranked_pathway_pairs_df_dict[excluded_layer][ranked_pathway_pairs_df_dict[excluded_layer]['# sig'] > 0]
            detected_ranked_pathway_pairs_dict[excluded_layer] = detected_ranked_pathway_pairs_df_dict[excluded_layer].index   
            len_sig_dict[excluded_layer] = len(ranked_pathway_pairs_df_dict[excluded_layer][ranked_pathway_pairs_df_dict[excluded_layer]['# sig'] > 0])
        ...
        for excluded_layer in tqdm(KEGG_interaction_types_dict.keys(), position=0, leave=True):
            ...
            for (p1, p2) in all_perms: 
                temp_df = shortest_paths_multilink_counts_stats_df_dict[(p1, p2)].loc[set(shortest_multilink_pos.keys()) - 
                                                                                      {'%s1' % KEGG_interaction_types_dict[excluded_layer], 
                                                                                       '%s-1' % KEGG_interaction_types_dict[excluded_layer]}]
                num_sig_multilinks.at['%s-%s' % (p1, p2), 'Pathway A'] = p1
                num_sig_multilinks.at['%s-%s' % (p1, p2), 'Pathway B'] = p2
    ...
    return (detected_ranked_pathway_pairs_df_dict, detected_ranked_pathway_pairs_dict, ranked_pathway_pairs_df_dict, 
            ranked_pathway_pairs_dict, len_sig_dict) 
